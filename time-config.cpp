...
// TODO via openbmc/openbmc#668 - openbmc/openbmc#1770 is still work in progress
// at the time of writiing this, so the use_dhcp_ntp is still using the old
// org.openbmc settings interfaces. The whole of time manager is anyway being
// rewritten to use new xyz.openbmc_project interfaces; as part of that,
// switching to new setting interfaces is also covered.
...
// is based around that fact. We use enums in the new settings interfaces, so
// code in this file can be changed to work with enums instead. That's being
// covered by the time manager rework (#668). For now, converting the settings
// to the string format that this object expects it to be.
...
int TimeConfig::settingsChanged(sdbusplus::message::message& msg)
{
    ...
    for(const auto& p : properties)
    {
        ...
        std::transform(setting.begin(), setting.end(), setting.begin(),
                       [](unsigned char c){ return std::toupper(c); });
        updatePropertyVal(path, setting);
    }
