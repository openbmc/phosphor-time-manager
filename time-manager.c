...
/* Reads host_time from /var/lib/obmc/host_time */
time_t read_host_data(const char *file_name)
{
	FILE *fp = NULL;
	time_t data = -1;
...
/* Writes the host_time / host_offset to file */
time_t write_host_data(const char *file_name, const time_t data)
{
	FILE *fp = NULL;
	int r = 0;
	...
	if (fwrite(&data, sizeof(data), 1, fp) != 1) {
		fprintf(stderr, "Error writing :[%ld] to [%s]\n",
				data, file_name);
		r = -1;
	}
	fclose(fp);
...
static int set_bmc_time(sd_bus_message *m, void *userdata, sd_bus_error *ret_error)
{
	...
	printf("set_bmc_time: curr_mode:[%d] curr_owner[%d]",
			g_curr_time_mode, g_curr_time_owner);
...
static int set_host_time(sd_bus_message *m, void *userdata, sd_bus_error *ret_error)
{
	...
	printf("set_host_time: curr_mode:[%d] curr_owner:[%d], curr_host_time:[%ld],"
			" host_offset:[%ld], req_host_time:[%ld]\n",
			g_curr_time_mode, g_curr_time_owner, g_curr_host_time,
			g_host_time_offset, new_host_time);
	...
	if (g_curr_time_owner == SPLIT) {
		...
		r = write_host_data(host_time_file, g_curr_host_time);
		if (r < 0)
			fprintf(stderr, "Error saving host_time:[%ld]\n",
					g_curr_host_time);
		/* FIXME : Does it make sense to abort ? */

		r = write_host_data(host_offset_file, g_host_time_offset);
		if (r < 0)
	...
	/* Mode must be either "Host" -or- "Both" */
				...
				uint32_t revents, void *userdata)
{
	...
	printf("process_time_change: curr_mode:[%d] curr_owner:[%d],"
...
int updt_time_mode(const time_mode_t time_mode)
{
	...
	printf("Requested_Mode:[%d], Current_Mode:[%d]\n",
			time_mode, g_curr_time_mode);
	...
	if (g_time_settings_changes_allowed) {
		...
		printf("Current_Mode changed to:[%d]\n",g_curr_time_mode);
	}
	return r;
}
...
int updt_time_owner(const time_owner_t time_owner)
{
	...
	printf("Requested_Owner:[%d], Current_Owner:[%d]\n",
			time_owner, g_curr_time_owner);
	...
	if (g_time_settings_changes_allowed) {
		...
		printf("Current_Owner changed to:[%d]\n",g_curr_time_owner);
	}
	return r;
}
...
int process_initial_settings(void)
{
	...
	if (init_pgood == -1) {
		fprintf(stderr, "Error reading  PGOOD value\n");
		g_time_settings_changes_allowed = false;
	} else if (!init_pgood) {
		g_time_settings_changes_allowed = true;
	}
	...
	g_curr_host_time = read_host_data(host_time_file);
	if (g_curr_host_time < 0) {
		fprintf(stderr, "Error reading current host time\n");
		goto finish;
	}

	/* Get current host_offset */
	g_host_time_offset = read_host_data(host_offset_file);
	if (g_host_time_offset < 0) {
