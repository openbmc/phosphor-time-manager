...
constexpr const char* matchPatternIpmbSelInfo =
                         ...
                         std::vector<uint8_t>& respData)
{
    ...
    try
    {
        for (uint8_t i = 0; i < hostTimeCmd.ipmbAddr.size(); i++)
        {
            method.append(hostTimeCmd.ipmbAddr[i], hostTimeCmd.netFn,
                          hostTimeCmd.lun, hostTimeCmd.cmd,
                          hostTimeCmd.cmdData);
            auto reply = bus.call(method);
            IpmbMethodType resp;
            reply.read(resp);
