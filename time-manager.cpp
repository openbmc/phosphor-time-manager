...
{
    config = std::make_unique<TimeConfig*>(timeConfig);
}

HostTime::HostTime(TimeConfig* timeConfig,
                   std::chrono::microseconds* hostOffset)
{
    config = std::make_unique<TimeConfig*>(timeConfig);
    iv_Offset = std::make_unique<std::chrono::microseconds*>(hostOffset);
}
                            ...
                            sd_bus_error* error)
{
    ...
    return sd_bus_message_append(m, "s",
                                 tmgr->config.modeStr(tmgr->config.getCurrTimeMode()));
}
                             ...
                             sd_bus_error* error)
{
    TimeManager *tmgr = TimeManager::getInstance();
    return sd_bus_message_append(m, "s",
                                 tmgr->config.ownerStr(tmgr->config.getCurrTimeOwner()));
}
                           ...
                           sd_bus_error* error)
{
    TimeManager *tmgr = TimeManager::getInstance();
    return sd_bus_message_append(m, "s",
                                 tmgr->config.modeStr(tmgr->config.getRequestedTimeMode()));
}
                            ...
                            sd_bus_error* error)
{
    TimeManager *tmgr = TimeManager::getInstance();
    return sd_bus_message_append(m, "s",
                                 tmgr->config.ownerStr(tmgr->config.getRequestedTimeOwner()));
}
                         ...
                         sd_bus_error* retError)
{
    ...
    else if (!strcasecmp(target, "host"))
    {
        ...
        {
            auto hostTime = dynamic_cast<HostTime*>(&time);
                                   ...
                                   uint32_t revents, void* userdata)
{
    ...
    std::cout <<" Curr_Mode: " << tmgr->config.modeStr(tmgr->config.getCurrTimeMode())
              << " Curr_Owner : " << tmgr->config.ownerStr(tmgr->config.getCurrTimeOwner())
    ...
    if (tmgr->config.getCurrTimeOwner() == tmgr->config.timeOwners::SPLIT)
    {
...
int TimeManager::processPgoodChange(sd_bus_message* m, void* user_data,
                                    sd_bus_error* retError)
{
    auto tmgr = TimeManager::getInstance();
                                       ...
                                       sd_bus_error* retError)
{
                ...
                "sv")) > 0)
    {
        ...
        else if (!strcmp(key, "time_owner"))
        {
            ...
            else if (tmgr->config.resetHostOffset())
            {
