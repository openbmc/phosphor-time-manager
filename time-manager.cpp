                ...
                sd_bus_error* retError)
{
    ...
    auto timeInSec = duration_cast<seconds>
                     (microseconds(timeInUsec)).count();

    std::cout << " host_time: [ " << hostTime.count()
              << " ] BMC_time: [ " << timeInUsec.count()
                ...
                sd_bus_error* retError)
{
    ...
    auto r = sd_bus_message_read(m, "x", &newHostTime);
    if (r < 0)
    {
        std::cerr << "Error: " << strerror(-r)
    ...
    auto hostTimeUsec = duration_cast<microseconds>(seconds(newHostTime));
    ...
    if (gCurrTimeOwner == timeOwners::SPLIT)
    {
        ...
        gHostOffset = hostTimeUsec - bmcTimeUsec;
        // We are not doing any time settings in BMC
        std::cout << "Updated: host_time: [  " << hostTimeUsec.count()
                  << " ] host_offset: " << gHostOffset.count()
                  << " ] " << std::endl;

        r = writeData<long long int>(hostOffsetFile, gHostOffset.count());
        if (r < 0)
        {
                      ...
                      uint32_t revents, void* userdata)
{
    ...
    if (gCurrTimeOwner == timeOwners::SPLIT)
    {
        ...
        std::cout << " G_UPTIME_USEC : " << gUptimeUsec.count();
        std::cout << " DELTA_TIME_USEC: " << deltaTimeUsec.count();
        std::cout << " PREV_HOST_OFFSET: " << gHostOffset.count()
                  << std::endl;
#endif
        // If the BMC time goes backwards, then - of - will handle that.
        gHostOffset = gHostOffset - deltaTimeUsec;

        std::cout << " UPDATED HOST_OFFSET: "
                  << gHostOffset.count() << std::endl;
        gUptimeUsec = uptimeUsec;

        // Persist this
        auto r = writeData<long long int>(hostOffsetFile, gHostOffset.count());
        if (r < 0)
        {
            std::cerr << "Error saving host_offset: "
                      << gHostOffset.count() << std::endl;
...
int updateTimeOwner(const std::string newOwnerStr)
{
    ...
    if (gTimeSettingsChangesAllowed)
    {
                ...
                gRequestedTimeOwner != timeOwners::SPLIT)
        {
            ...
            writeData<long long int>(hostOffsetFile, gHostOffset.count());
            if (r < 0)
            {
                std::cerr << "Error saving host_offset: "
                          << gHostOffset.count() << std::endl;
...
int readPersistentData()
{
    ...
    auto hostTimeOffset = readData<long long int>(hostOffsetFile);
    gHostOffset = microseconds(hostTimeOffset);
    ...
    std::cout <<"Initial Uptime Usec: " << gUptimeUsec.count() << std::endl;
